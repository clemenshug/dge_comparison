...
results_meta <- results %>%
  select(name, condition, method, cell_line, drug, concentration, time) %>%
  distinct() %>%
...
clust_cols <- hclust(
  as.dist(1 - cor(lfc_matrix_cdk, method = "spearman")),
  # dist(t(lfc_log_model_matrix), method = "euclidian"),
  method = "average"
)

clust_rows <- hclust(
  as.dist(1 - cor(t(lfc_matrix_cdk), method = "spearman")),
...
ggsave(file.path("deseq", "cdk_log2fc_hc_spearman_average.pdf"), lf_hm_cdk, width = 10, height = 10)
```
