...
instance_meta <- readr::read_tsv("~/data/DGE_comp/GSE92742_Broad_LINCS_inst_info.txt", na = "-666")
gene_meta <- readr::read_tsv("~/data/DGE_comp/GSE92742_Broad_LINCS_gene_info.txt", na = "-666")
pert_meta <- readr::read_tsv(
  "~/data/DGE_comp/GSE92742_Broad_LINCS_pert_info.txt",
  col_types = "ccclcccc", na = "-666"
)

cmap_pubchem_ids <- instance_meta %>%
  distinct(pert_id) %>%
  left_join(pert_meta, by = "pert_id")

cdk_pubchem_ids <- cdk_meta_combined %>%
  distinct(drug) %>%
  mutate(
...
shared_conditions <- cdk_meta_combined %>%
  left_join(cdk_pubchem_ids, by = "drug") %>%
  left_join(cmap_pubchem_ids, by = c("pubchem_id" = "pubchem_cid"))
