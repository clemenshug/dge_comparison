...
marginal_ecdf_fraction_detectable <- fraction_detectable_per_gene %>%
  group_by(l1000, comparison) %>%
  do(
    plot = (function (df, comparison) {
      p <- ggplot(df, aes(fraction_detectable)) +
        geom_histogram(aes(y = stat(count)/sum(stat(count))), breaks = seq(from = 0, to = 1, length.out = 10), closed = "right") +
        stat_ecdf(geom = "step") +
        # geom_density() +
        labs(
          x = NULL,
        ...
        lims(x = c(0, 1))
      if (df$comparison[1] == "dge") p <- p + scale_y_reverse() + coord_flip()
      p
    })(.)
  )
...
fraction_detectable_per_gene_density_scatter <- fraction_detectable_per_gene %>%
  ...
  split(.$l1000) %>%
  map(mutate, density = density_2d(bulk, dge)) %>%
  # Arrange by density so that points with highest density are on top of plot
  map(arrange, density) %>%
  imap(
    function(df, gene_set) {
      p <- ggplot(df, aes(bulk, dge, color = density)) +
        geom_point() +
        coord_fixed() +
      ...
      egg::ggarrange(
        plots = list(
          ggplot() + theme_void(),
          filter(marginal_ecdf_fraction_detectable, l1000 == gene_set, comparison == "bulk")$plot[[1]],
          filter(marginal_ecdf_fraction_detectable, l1000 == gene_set, comparison == "dge")$plot[[1]],
          p
        ),
        widths = c(1, 4),
        heights = c(1, 3),
