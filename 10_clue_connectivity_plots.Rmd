...
```{r load}
compound_name_map <- synapser::synGet("syn21586544", version = 1) %>%
  chuck("path") %>%
  read_csv() %>%
  distinct(
    lspci_id,
...
dose_response_curve <- function(df) {
  df %>%
    ggplot(aes(drug_conc, change, color = method)) +
    geom_point() +
    geom_smooth(aes(fill = method, group = method)) +
    scale_x_log10() +
    labs(x = "Dose (uM)", y = "Expression change (z-score)")
}

dose_response_cor_and_curves <- function(df, seed = 42) {
  set.seed(seed)
  ...
  arrangeGrob(
    ...
    widths = unit(c(3, 4, 3), "in"),
    heights = unit(c(2, 2), "in")
  )
}
...
palbo_dose_cor_plot <- dose_response_cor_and_curves(palbo_dose_cor, seed = 1)
