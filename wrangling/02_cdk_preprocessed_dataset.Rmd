...
cdk_bulk_meta <- list(
  "201608" = syn_tsv("syn10155361") %>%
    dplyr::mutate(DrugName = dplyr::recode(DrugName, `-` = "DMSO")) %>%
...
cdk_dge_sample_counts <- cdk_dge_counts %>%
  dplyr::group_by(sample_id) %>%
  dplyr::summarize(count = sum(count)) %>%
  dplyr::arrange(count)

cdk_dge_sample_counts_plot <- cdk_dge_sample_counts %>%
  dplyr::mutate(sample_id = factor(sample_id, levels = unique(sample_id))) %>% 
  ggplot(aes(sample_id, count)) +
  geom_col()

cdk_bulk_sample_counts <- cdk_bulk_counts %>%
  dplyr::group_by(sample_id) %>%
  dplyr::summarize(count = sum(count)) %>%
  dplyr::arrange(count)

cdk_bulk_sample_counts_plot <- cdk_bulk_sample_counts %>%
  dplyr::mutate(sample_id = factor(sample_id, levels = unique(sample_id))) %>% 
  ggplot(aes(sample_id, count)) +
  geom_col()
```
...
hsap <- biomaRt::useMart("ENSEMBL_MART_ENSEMBL", "hsapiens_gene_ensembl")

ensembl_gene_id_mart <- biomaRt::getBM(
  attributes = c("hgnc_symbol", "ensembl_gene_id"),
  filters = "ensembl_gene_id",
  values = union(cdk_dge_counts$ensembl_gene_id, cdk_bulk_counts$ensembl_gene_id),
  mart = hsap
) %>%
  dplyr::rename(symbol = hgnc_symbol)

ensembl_gene_id_mart %>%
  dplyr::count(ensembl_gene_id) %>%
  dplyr::count(n)
# Two ensembl_gene_ids map to multiple HUGO symbols, setting those NA...
# Also there are some ensembl_gene_ids that are in the biomart DB, but that
# map to empty "" symbols. Setting those to NA as well.
ensembl_gene_id_mapping <- ensembl_gene_id_mart %>%
  dplyr::group_by(ensembl_gene_id) %>%
  dplyr::summarize(symbol = if (n() > 1) as.character(NA) else symbol) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(symbol = ifelse(symbol == "", as.character(NA), symbol))
ensembl_gene_id_mapping %>%
  dplyr::count(ensembl_gene_id) %>%
  dplyr::count(n)
# Good now, no duplicates

all(cdk_common_ids %in% ensembl_gene_id_mapping$ensembl_gene_id)
# TRUE
# All ensembl IDs are present in the biomart df!

cdk_bulk_counts <- cdk_bulk_counts %>%
  dplyr::left_join(ensembl_gene_id_mapping, by = "ensembl_gene_id")
...
cdk_meta_combined <- list(
  ...
  dplyr::mutate(sample_id = paste_(method, sample_id)) %>%
...
```{r writing_data_disk}
readr::write_csv(cdk_bulk_meta, file.path("..", "wrangled", "cdk_bulk_meta.csv"))
readr::write_csv(cdk_bulk_counts, file.path("..", "wrangled", "cdk_bulk_counts.csv.gz"))

readr::write_csv(cdk_dge_meta, file.path("..", "wrangled", "cdk_dge_meta.csv"))
readr::write_csv(cdk_dge_counts, file.path("..", "wrangled", "cdk_dge_counts.csv.gz"))

readr::write_csv(cdk_meta_combined, file.path("..", "wrangled", "cdk_meta_combined.csv"))
readr::write_csv(cdk_count_combined, file.path("..", "wrangled", "cdk_count_combined.csv.gz"))

readr::write_rds(matched_samples, file.path("..", "wrangled", "cdk_meta_matched_samples.rds"))

readr::write_csv(ensembl_gene_id_mapping, file.path("..", "wrangled", "ensembl_gene_id_mapping.csv"))
```
