...
comp_res_df <- comp_res %>%
  select(name, method, cell_line, drug, concentration, time, batch, experiment, condition, result)
readr::write_rds(comp_res_df, file.path("deseq", "deseq_treatment_vs_control_result.rds"))

# n_workers = 10
# tryCatch({
#     compute_cluster <- parallel::makeForkCluster(n_workers, outfile = "deseq_worker.log")
#     split_comp_deseqs <- comp_deseqs %>%
#       base::split(rep_len(1:n_workers, nrow(.)))
#     split_comp_res <- pbapply::pblapply(
#       split_comp_deseqs,
#       function(chunk) {
#         library(DESeq2)
#         library(tidyverse)
